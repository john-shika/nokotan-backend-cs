...
public sealed class AutoGenerateDescriptionTransformer(IAuthenticationSchemeProvider authenticationSchemeProvider) : IOpenApiDocumentTransformer
{
    ...
    // suppressing warning debug
    private object?[] _ => [authenticationSchemeProvider];
    ...
    public Task TransformAsync(OpenApiDocument document, OpenApiDocumentTransformerContext context, CancellationToken cancellationToken)
    {
        ...
        foreach (var (path, item) in document.Paths)
        {
            ...
            {
                if (NokoCommonMod.IsNoneOrEmptyWhiteSpace(operation.Summary)) continue;
                Logger.Warning($"Set Description On Method: {operationType}, Path: '{path}'");
                var value = NokoCommonMod.EndsCutString(operation.Summary.Trim(), ".");
                var summary = NokoTransformText.ToTitleCase(value.ToLower());
